...
class TestMemoryStore:
    def test_save_and_load(self, temp_store):
        ...
        temp_store.save(ev)
        loaded = temp_store.load("Tic-Tac-Toe")

        assert loaded is not None
        assert loaded.game_name == "Tic-Tac-Toe"
        assert loaded.weights == [0.1, 0.2, 0.3, 0.4]
    ...
    def test_exists(self, temp_store):
        assert not temp_store.exists("Tic-Tac-Toe")
        ev = LearnableEval("Tic-Tac-Toe")
        temp_store.save(ev)
        assert temp_store.exists("Tic-Tac-Toe")

    def test_list_games_empty(self, temp_store):
        assert temp_store.list_games() == []

    def test_list_games(self, temp_store):
        temp_store.save(LearnableEval("Tic-Tac-Toe"))
        temp_store.save(LearnableEval("Connect Four"))
    ...
    def test_delete(self, temp_store):
        temp_store.save(LearnableEval("Tic-Tac-Toe"))
        assert temp_store.exists("Tic-Tac-Toe")
        assert temp_store.delete("Tic-Tac-Toe")
        assert not temp_store.exists("Tic-Tac-Toe")
    ...
    def test_overwrite(self, temp_store):
        ev1 = LearnableEval("Tic-Tac-Toe", weights=[0.1, 0.2, 0.3, 0.4])
        ev1.generation = 10
        temp_store.save(ev1)

        ev2 = LearnableEval("Tic-Tac-Toe", weights=[0.5, 0.6, 0.7, 0.8])
        ev2.generation = 20
        temp_store.save(ev2)

        loaded = temp_store.load("Tic-Tac-Toe")
        assert loaded.weights == [0.5, 0.6, 0.7, 0.8]
    ...
    def test_isolation_between_games(self, temp_store):
        ...
        ev_ttt = LearnableEval("Tic-Tac-Toe", weights=[1.0, 2.0, 3.0, 4.0])
        ev_ttt.generation = 50
        temp_store.save(ev_ttt)

        ev_c4 = LearnableEval("Connect Four", weights=[5.0, 6.0, 7.0, 8.0])
        ev_c4.generation = 30
        temp_store.save(ev_c4)

        # Loading TTT should still have original weights
        loaded_ttt = temp_store.load("Tic-Tac-Toe")
        assert loaded_ttt.weights == [1.0, 2.0, 3.0, 4.0]
        assert loaded_ttt.generation == 50

        # C4 should have its own weights
        loaded_c4 = temp_store.load("Connect Four")
        assert loaded_c4.weights == [5.0, 6.0, 7.0, 8.0]
